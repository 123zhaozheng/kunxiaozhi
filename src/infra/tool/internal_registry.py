...
def build_internal_tools(*, include_sandbox_tools: bool | None = None) -> list[BaseTool]:
    ...
    # API key) and the rerank model configuration. Missing any of these means
    # the tools cannot function, so they are not exposed.
    if (
        ...
        # Register two separate tools for agent control: list KBs first, then query
        tools.append(get_list_dify_knowledge_bases_tool())
        tools.append(get_query_dify_knowledge_base_tool())
