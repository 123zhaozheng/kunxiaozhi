    ...
    <>
      ...
      <div className="flex flex-col gap-px px-2 py-2 space-y-1">
        ...
        <button
          ...
          className={`sidebar-nav-btn w-full h-8 rounded-[10px] flex items-center gap-3 px-[9px] focus:outline-none transition-colors ${isWorkspaceActive ? "is-active" : ""}`}
          aria-current={isWorkspaceActive ? "page" : undefined}
        >
          <Sparkles size={20} />
          <span>{t("nav.workspace", "专家 · 技能 · 连接器")}</span>
        </button>
        ...
        >
          <div className="shrink-0 w-8 h-8 rounded-full overflow-hidden ring-1 ring-stone-200 dark:ring-stone-700 group-hover:ring-[var(--theme-primary)] transition mr-3">
            {user?.avatar_url && !imgError ? (
              ...
              <div className="flex w-full h-full items-center justify-center bg-[radial-gradient(circle,#ffffff_25%,#bbf7d0_100%)] rounded-full">
